...
setwd(dataFolder)
if('imputed_intersected.RData' %in% dir()){
  load('imputed_intersected.RData')
}else{
...
load_incomplete_data <- function(cancer){
  ...
  incomplete_data <- incompleteIntersection$incomplete
  ...
  incomplete_ind <- feature_subset_indices(incomplete_data)
  incomplete_data <- subset_data(incomplete_data, incomplete_ind)
  ...
  incomplete_stat <- row_statistics(incomplete_data)
  incomplete_data <- normalize_data(incomplete_data, incomplete_stat)
  return(list(first = incomplete_data, second = clinical_data))
  
}

#### Load in cases txt files that have 2000 features and are normalized 
brca <- load_incomplete_data(cancer = 'BRCA')
kirc <- load_incomplete_data(cancer = 'KIRC')
lihc <- load_incomplete_data(cancer = 'LIHC')
luad <- load_incomplete_data(cancer = 'LUAD')
...
save.image('imputed_intersected.RData')
}
