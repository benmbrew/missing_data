...
# subset to SNF
int_results <- int_results[grepl('SNF', int_results$method),]
union_results <- union_results[grepl('SNF', union_results$method),]
...
top_int_acc <- int_results %>%
  ...
  summarise(top1_acc_nmi = sum(acc_nmi_rank == 1),
            top2_acc_nmi = sum(acc_nmi_rank == 2),
            top3_acc_nmi = sum(acc_nmi_rank == 3),
            top4_acc_nmi = sum(acc_nmi_rank == 4),
            top5_acc_nmi = sum(acc_nmi_rank == 5),
            top6_acc_nmi = sum(acc_nmi_rank == 6),
            top7_acc_nmi = sum(acc_nmi_rank == 7))
...
ggplot(data = top_int_acc_melt, aes(reorder(method, -value), value, fill = variable)) +
  ...
  scale_fill_manual(values = c("grey8", "grey25", "grey40","grey55","grey60","grey75","grey90"), 
                    name="ACC and NMI Ranking",
                    breaks=c("top1_acc_nmi", "top2_acc_nmi", "top3_acc_nmi", "top4_acc_nmi", "top5_acc_nmi","top6_acc_nmi","top7_acc_nmi"),
...
top_int_pval <- int_results %>%
  ...
  summarise(top1_pval = sum(pval_ci_rank == 1),
            top2_pval = sum(pval_ci_rank == 2),
            top3_pval = sum(pval_ci_rank == 3),
            top4_pval = sum(pval_ci_rank == 4),
            top5_pval = sum(pval_ci_rank == 5),
            top6_pval = sum(pval_ci_rank == 6),
            top7_pval = sum(pval_ci_rank == 7))
...
ggplot(data = top_int_pval_melt, aes(reorder(method, -value), value, fill = variable)) +
  ...
  scale_fill_manual(values = c("grey8", "grey25", "grey40","grey55","grey60","grey75","grey90"), 
                    ...
                    breaks=c("top1_pval", "top2_pval", "top3_pval", "top4_pval", "top5_pval","top6_pval","top7_pval"),
                    labels=c("Rank 1", "Rank 2", "Rank 3","Rank 4","Rank 5","Rank 6","Rank 7")) + 
  scale_y_continuous(breaks=c(1,2,3,4,5)) +
  theme(panel.background=element_rect(fill="white"), 
        plot.background=element_rect(fill="white"), 
...
top_union <- union_results %>%
  ...
  summarise(top1_pval = sum(total_rank == 1),
            top2_pval = sum(total_rank == 2),
            top3_pval = sum(total_rank == 3),
            top4_pval = sum(total_rank == 4),
            top5_pval = sum(total_rank == 5),
            top6_pval = sum(total_rank == 6),
            top7_pval = sum(total_rank == 7))
...
ggplot(data = top_union_melt, aes(reorder(method, -value), value, fill = variable)) +
  ...
  scale_y_continuous(breaks=c(1,2,3,4,5)) +
  ggtitle('Union') +
  scale_fill_manual(values = c("grey8", "grey25", "grey40","grey55","grey60","grey75","grey90"), name="Pval and CI Ranking",
                    breaks=c("top1_pval", "top2_pval", "top3_pval", "top4_pval", "top5_pval","top6_pval","top7_pval"),
                    labels=c("Rank 1", "Rank 2", "Rank 3","Rank 4","Rank 5","Rank 6","Rank 7")) +
  theme(panel.background=element_rect(fill="white"), 
        plot.background=element_rect(fill="white"), 
