...
# rename impute values 
impute$cancer <- cancerTypes[impute$cancer]
impute$impute <- imputeTypes[impute$impute]

# which imputation method had highest and lowest nrmse 
impute$type <- interaction(impute$cancer, impute$impute, drop=TRUE)

# group by type and get mean of methyl, mirna, mrna
grouped_data <- impute %>%
  group_by(type) %>%
  summarise(methyl = mean(methyl),
            mirna = mean(mirna),
            mrna = mean(mrna))
