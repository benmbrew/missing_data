...
######################################################################
# Subset clinical data and union by gender
...
for (i in 1:numViews) {
  ...
  colnames(temp.unionData) <- transformIDFormat(colnames(temp.unionData))
  unionDataInd <- match(genderIDs, as.character(colnames(temp.unionData)))
  temp.unionData <- temp.unionData[,unionDataInd]
  unionData[[i]] <- temp.unionData
}
...
clinicalDataInt <- clinicalData[clinicalInd, ]

######################################################################
# Subset clinical data and completeData by gender
# Subset clinical data and union by gender
clinGenderInt <- clinicalDataInt[clinicalDataInt$gender == gender,]
clinGenderInt <- clinGenderInt[rowSums(is.na(clinGenderInt)) < ncol(clinGenderInt),]
genderIDsInt <- as.character(clinGenderInt$bcr_patient_barcode)

for (i in 1:numViews) {
  temp.intersectedData <- intersectedData[[i]]
  colnames(temp.intersectedData) <- transformIDFormat(colnames(temp.intersectedData))
  intersectedDataInd <- match(genderIDsInt, as.character(colnames(temp.intersectedData)))
