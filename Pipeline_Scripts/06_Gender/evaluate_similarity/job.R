...
# make clinical time numeric
clinicalData$days_to_death <- as.numeric(as.character(clinicalData$days_to_death))
clinicalData$days_to_last_followup <- as.numeric(as.character(clinicalData$days_to_last_followup))

######################################################################
# Subset clinical data and completeData by gender
clinGender <- clinicalData[clinicalData$gender == gender,]
clinGender <- clinGender[rowSums(is.na(clinGender)) < ncol(clinGender),]
genderIDs <- as.character(clinGender$bcr_patient_barcode)
...
for (i in 1:numViews) {
  ...
  colnames(temp.intersectedData) <- transformIDFormat(colnames(temp.intersectedData))
  intersectedDataInd <- match(genderIDs, as.character(colnames(temp.intersectedData)))
  temp.intersectedData <- temp.intersectedData[,intersectedDataInd]
  temp.intersectedData <- removeNASamples(temp.intersectedData)
  intersectedData[[i]] <- temp.intersectedData
}

######################################################################
# Subset incompleteData and removedData by gender
for (i in 1:numViews) {
  temp.incompleteData <- incompleteData[[i]]
  colnames(temp.incompleteData) <- transformIDFormat(colnames(temp.incompleteData))
  incompleteDataInd <- match(genderIDs, as.character(colnames(temp.incompleteData)))
  temp.incompleteData <- temp.incompleteData[,incompleteDataInd]
  incompleteData[[i]] <- temp.incompleteData
}

for (i in 1:numViews) {
  temp.removedData <- removedData[[i]]
  colnames(temp.removedData) <- transformIDFormat(colnames(temp.removedData))
  removedDataInd <- match(genderIDs, as.character(colnames(temp.removedData)))
  temp.removedData <- temp.removedData[,removedDataInd]
  removedData[[i]] <- temp.removedData
}
...
similarityData <- list(incompleteData, intersectedData)

# Read in the SNF cluster labels for the complete data
fileName <- paste(genderInd, "_3.txt", sep="")
