...
combinedScoresMale <- combinedScores[combinedScores$gender == 1,]
combinedScoresFemale <- combinedScores[combinedScores$gender == 2,]

# Write combinedScores to results folder 
write.csv(combinedScores, paste0(results_folder, '/scoresGender.csv'))
...
testScoresBoth <- matrix(, 0, 6)

for (gen in 1:length(genderTypes)) {
  for (met1 in 1:length(methodTypes)) {
    score1 <- subset(combinedScores, gender==gen&method==met1)
    accVector <- rep.int(0, length(methodTypes))
    nmiVector <- accVector
    pvalVector <- accVector
    ciVector <- accVector
    for (met2 in (1:length(methodTypes))[-met1]) {
      score2 <- subset(combinedScores, gender==gen&method==met2)
      accVector[met2] <- xGreaterY(score1$acc, score2$acc)
      nmiVector[met2] <- xGreaterY(score1$nmi, score2$nmi)
      pvalVector[met2] <- xGreaterY(score1$pval, score2$pval)
      ciVector[met2] <- xGreaterY(score1$ci, score2$ci)
    }
    testScoresBoth <- rbind(testScoresBoth,
                            c(gen, met1, sum(accVector), sum(nmiVector),
                              sum(pvalVector), sum(ciVector)))
  }
}

colnames(testScoresBoth) <- c("gender", "method", "acc", "nmi", "pval","ci")
testScoresBoth <- as.data.frame(testScoresBoth)
testScoresBoth$gender <- genderTypes[testScoresBoth$gender]
testScoresBoth$method <- methodTypes[testScoresBoth$method]

write.table(testScoresBoth, "testScoresBoth.txt", row.names=FALSE)
