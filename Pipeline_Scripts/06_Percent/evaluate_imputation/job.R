...
# Imputation method
runType <- argv[2]
# index for which membership we dont set equal to true
member <- argv[3]
# index for which percent we remove 
percent <- argv[4]
...
# Generate an incomplete data set by randomly removing samples from each data type. 

set.seed(seed)

incompleteIntersection <- generateIncompleteIntersectionPercent(cases)
