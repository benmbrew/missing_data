...
scripts_folder <- paste(project_folder, "Scripts", "missing_data_all", sep = "/")
...
clusterCols <- c("cancer", "impute", "cluster_size", "acc",
...
similarityCols <- c("cancer", "similarity", "cluster_size", "acc",
                    "nmi", "pval", "ci", "se.std", "pvalcox", "coefcox", "con_index_ci",
                    "con_index_p", "bias_corrected_c_index")

similarity <- loadResults("evaluate_original_similarity", "similarity.txt", similarityCols)
...
write.csv(combinedScores, paste0(results_folder, '/missing_data_orig_all.csv'))
...
testScores5_orig <- byCluster(combinedScores, 1)
testScores4_orig <- byCluster(combinedScores, 2)
testScores3_orig <- byCluster(combinedScores, 3)
testScores2_orig <- byCluster(combinedScores, 4)
