...
for (i in 1:numViews) {
  ...
  unionData[[i]] <- temp.2.unionData
}

temp.dat <- unionData[[1]]
...
fileName <- paste(cancerInd, "_3", sep="")
for (i in 1:length(dataTypes)) {
  newName <- paste(fileName, "_", i, ".txt", sep="")
  ...
  for (j in 1:length(similarityMethods)) {
    ...
    for (k in 1:length(similarityData)) {
      data <- similarityData[[k]]
      ...
      similarityResults <- evaluateSimilarity(similarity, data,
                                              ...
                                              sampleRows)
      writeResults(c(argv, i, j, k, similarityResults), similarityFile)
      }
   }
}
