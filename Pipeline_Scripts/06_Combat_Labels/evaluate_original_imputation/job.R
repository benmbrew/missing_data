...
for (i in 1:length(clusters)) {
  for (j in 1:length(clusteringMethods)) {
    # Read in the cluster labels for the complete data
    fileName <- paste(cancerInd, "_", i, "_", j, ".txt", sep="")
    ...
    cluster <- function(x) clusteringMethods[[j]](x, numClus,
                                                  sampleRows)
    
    data <- imputedData
    tcgaID <- function(x) colnames(x[[1]])
    dataInd <- match(tcgaID(data), tcgaID(imputedData))
    clusteringResults <- evaluateClusteringLab(cluster, data,
