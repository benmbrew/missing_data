...
if (cancer == 'KIRC') {
  ...
  clinicalData <- clinicalData[rowSums(is.na(clinicalData)) < ncol(clinicalData),]
  clinicalIDs <- as.character(clinicalData$bcr_patient_barcode)
  intersectedInd <- match(clinicalIDs, intersectedIDs)
  ...
  for (i in 1:numViews) {
    ...
    temp.intersectedData <- temp.intersectedData[,intersectedInd]
...
for (i in 1:length(clusteringMethods)) {
  ...
  numClus <- length(unique(completeLabelsInt))
  ...
  for (j in 1:length(clusteringData)) {
    ...
    clusteringResults <- evaluateClusteringInt(cluster, data,
